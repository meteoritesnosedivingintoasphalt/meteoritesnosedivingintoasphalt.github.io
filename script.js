...
function inputReusesTargetWord(input, target, acceptedWords = []) {
  ...
  // Multi-word targets (e.g. "as a rule"): retyping the whole phrase counts as reuse.
  if (targetNorm.includes(" ") && normalize(input).includes(targetNorm)) return true;
...
function renderWord() {
  ...
  // Long phrases get a smaller heading so they stay readable.
  els.targetWord.classList.toggle("long", entry.word.length > 22);
