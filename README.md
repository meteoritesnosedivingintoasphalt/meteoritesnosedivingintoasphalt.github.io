...
Multi-word entries (e.g. `as a rule`, `walk the talk`) work the same way:
put the whole phrase in `word` and wrap it in `**double asterisks**` in the
context sentence. Retyping the full phrase is treated as reusing the original,
and headings longer than 22 characters are shown in a smaller size.
